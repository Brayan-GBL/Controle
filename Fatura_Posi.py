...
def processar_analise(cobranca_file, triagem_file):
    ...
    triagem_df["PALLET"] = triagem_df["PALLET"].astype(str).str.strip()

    # Criar chave de concatenação na base Cobrança (NF + LOCAL)
    cobranca_df["CHAVE"] = cobranca_df["NF"] + "_" + cobranca_df["LOCAL"]

    # Preencher valores ausentes antes de agrupar (evita problemas de soma)
    triagem_df["QTDE FÍSICA (BOM)"] = triagem_df["QTDE FÍSICA (BOM)"].fillna(0)
    triagem_df["QTDE FÍSICA (RUIM)"] = triagem_df["QTDE FÍSICA (RUIM)"].fillna(0)

    # Criar chave de concatenação na base Triagem (NOTA FISCAL + PALLET)
    triagem_df["CHAVE"] = triagem_df["NOTA FISCAL"] + "_" + triagem_df["PALLET"]

    # Consolidar quantidades físicas (BOA + RUIM) da triagem considerando NF e PALLET
    triagem_consolidado = triagem_df.groupby("CHAVE").agg({
        "QTDE FÍSICA (BOM)": "sum",
    ...
    # Mesclar os dados usando a CHAVE
    resultado_df = cobranca_df.merge(triagem_consolidado, on="CHAVE", how="left")

    # Garantir que não haja valores NaN após o merge
    cols_preencher = ["CONCAT_DEV", "QTDE FÍSICA (BOM)", "QTDE FÍSICA (RUIM)"]
    for col in cols_preencher:
        if col in resultado_df.columns:
            resultado_df[col].fillna(0, inplace=True)
    ...
    def classificar_diferenca(row):
        bom_ruim_soma = row["QTDE FÍSICA (BOM)"] + row["QTDE FÍSICA (RUIM)"]
        if row["CONCAT_DEV"] > row["QTD UND"] and row["CONCAT_DEV"] == bom_ruim_soma:
            return "Informação incorreta - Devemos pagar mais"
        elif row["DIFERENÇA"] > 0 and bom_ruim_soma < row["QTD UND"]:
